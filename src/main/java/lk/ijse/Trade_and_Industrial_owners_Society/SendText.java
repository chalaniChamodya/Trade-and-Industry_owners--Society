    ...
    private static Credential getCredentials(final NetHttpTransport HTTP_TRANSPORT, GsonFactory gsonFactory)
        ...
        GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(gsonFactory,
                new InputStreamReader(SendText.class.getResourceAsStream
                        ("/API/credential.json"))); // enter your credential
...
//        try {
//            new SendText().sendMail("hii","hello","chalanichamodya2003@gmail.com");
//        } catch (IOException e) {
...
//    }
    ...
    public void sendMail(String subject, File QR, String TEST_MAIL) throws  IOException, MessagingException {
        this.TEST_MAIL=TEST_MAIL;
        ...
        email.addRecipient(javax.mail.Message.RecipientType.TO, new InternetAddress(TEST_MAIL));
        email.setSubject(subject);
        //email.setContent(QR,"Your QR code !");
        MimeBodyPart msgbodyPrt = new MimeBodyPart();
        msgbodyPrt.setText("Please find the QR code for attendance below !");

        MimeBodyPart attachmentPrt = new MimeBodyPart();
        DataSource source = new FileDataSource(QR);
        attachmentPrt.setDataHandler(new DataHandler(source));
        attachmentPrt.setFileName("QR.png");

        Multipart multipart = new MimeMultipart();
        multipart.addBodyPart(msgbodyPrt);
        multipart.addBodyPart(attachmentPrt);
